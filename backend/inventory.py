...
def add_component(part_number: str, component_type: str, value: str, quantity: int,
    ...
    if not part_number:
        raise InvalidInputError("Part number cannot be empty.")
    if quantity < 0:
    ...
    try:
        ...
        session.rollback()
        if isinstance(e, (ComponentError, ValueError)):
             raise
        raise DatabaseError(f"Database error during add: {e}") from e
    finally:
        session.close()
...
def remove_component_quantity(component_id: uuid.UUID, quantity: int) -> Component | None:
    ...
    try:
        ...
        if component.quantity < quantity:
            raise StockError(f"Not enough stock for component {component.part_number} (ID: {component_id}). "
...
def get_component_by_id(component_id: uuid.UUID) -> Component | None:
    ...
    try:
        ...
        raise DatabaseError(f"Error while fetching component by id {component_id}: {e}") from e
    finally:
        session.close()
...
def get_all_components() -> list[Component]:
    ...
    try:
        ...
        raise DatabaseError(f"Error while fetching all components: {e}") from e
    finally:
        session.close()
...
def get_components_by_part_number(part_number: str) -> list[Component]:
    ...
    try:
        ...
        raise DatabaseError(f"Error fetching components by part number {part_number}: {e}") from e
    finally:
        session.close()